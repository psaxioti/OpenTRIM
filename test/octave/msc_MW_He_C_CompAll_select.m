...
% load SRIM data
folders = {'../srim/msc/FC/HeinC/', '../srim/msc/monolayer/HeinC/' };
for m = 1:2,
  cd(folders{m});
  addpath('../../')
...
% load mcnp data
MCNP = load('../mcnp/mcnp_data_he2.dat');
M_theta = MCNP(:,1);
M_fnal1 = MCNP(:,2);
...
semilogy(M_theta,M_fnal1,':','linewidth',2)% mcnp
hold off
xlabel('angle (rad)')
ylabel('Prob. Density (sr^{-1})')
lbls2 = {'ML', 'd/100', 'MH, T>1eV',...
        'FC','monolayer','mcnp'};
legend(lbls2)
title('270 keV He^+ in 100\mum/cm^2 C')
grid

%print2png(gcf,[4 3]*4,'msc_HeinC_all_select')

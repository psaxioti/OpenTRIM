...
int InitConfiguration(char* ConfigFileName){
  ...
  float random;
  int i;
  char *default_configfile = "default.config";
  ...
  /* Read settings from default config file, if it exists: */
  if(file_readable(default_configfile)==1){
    message(-1,"Default config file found. \n");
    result=IniFileReader(ConfigFileDataBlockReader, ConfigFileDataReader, default_configfile);
    if(result!=0){message_error(-1,"Error reading default config file %s.\n",default_configfile);}
  } else {
    message(-1,"Default config not found: %s\n",default_configfile);
  }

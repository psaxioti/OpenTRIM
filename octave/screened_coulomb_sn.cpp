...
double exec_func(double x, double y = M_PI)
{
  ...
  if ((x <= 0.) || (y <= 0.)) return std::numeric_limits<double>::infinity();

  return xs_quad<Screening::ZBL>::stoppingPower(x,y);
}

DEFUN_DLD (screened_coulomb_sn, args, nargout, "Center mass scattering angle for screened Coulomb potential")
{
  if (args.length() > 2)
    print_usage ();

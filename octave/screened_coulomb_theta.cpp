...
{  
  return (std::isnan(x) || std::isnan(y)) ? std::numeric_limits<double>::quiet_NaN() : xs_quad<Screening::ZBL>::theta(x,y);
}

DEFUN_DLD (screened_coulomb_theta, args, nargout, "Center mass scattering angle for screened Coulomb potential")
{
  if (args.length () != 2)
    print_usage ();
